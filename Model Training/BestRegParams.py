...
def RunCNN(seqs):
    ...
    model = Sequential()
    model.add(Conv2D(filters = 580, kernel_size = (20, 2), strides = 1, activation = 'relu', input_shape = (1000, 4, 1)))
    ...
    model.add(BatchNormalization())
    model.add(Conv2D(filters = 580, kernel_size = (5, 2), activation = 'relu'))
    model.add(MaxPooling2D(pool_size = (3, 1)))
    model.add(BatchNormalization())
    model.add(Flatten())
    model.add(Dense(1150, activation = 'relu', kernel_regularizer=regularizers.l1_l2(l1 = 0.1, l2 = 0.1)))
    model.add(Dense(1, activation = 'sigmoid', kernel_regularizer=regularizers.l1_l2(l1 = 0.1, l2 = 0.1)))
    # Changing learning rate; default is 0.001
    optimizer = keras.optimizers.Adam(lr = 0.01)
    model.compile(loss = 'binary_crossentropy', optimizer = optimizer, metrics = ['accuracy'])
    print("TRAINING:")
    final = model.fit(x_train, y_train, batch_size = 100, epochs = 10, verbose = 2)
    print("VALIDATION:")
    validate = model.evaluate(x_validate, y_validate, verbose=2)
    model.save('MouseBestRegParamsDropped.h5')
