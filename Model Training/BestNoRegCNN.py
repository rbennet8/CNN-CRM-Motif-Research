...
def RunCNN(seqs):
    ...
    model = Sequential()
    model.add(Conv2D(filters = 420, kernel_size = (20, 2), strides = 1, activation = 'relu', input_shape = (1000, 4, 1)))
    model.add(MaxPooling2D(pool_size = (10, 1)))
    model.add(BatchNormalization())
    model.add(Conv2D(filters = 360, kernel_size = (15, 2), activation = 'relu'))
    model.add(MaxPooling2D(pool_size = (7, 1)))
    model.add(BatchNormalization())
    model.add(Conv2D(filters = 400, kernel_size = (5, 2), activation = 'relu'))
    ...
    model.save('MouseBestNoRegCNNDropped.h5')
